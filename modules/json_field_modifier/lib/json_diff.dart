...
class JsonDiffPatcher {
  static JsonPatch diff(dynamic source, dynamic target) {
  ...
  static void applyPatch(dynamic source, JsonPatch patch) {
...
void main() {
  ...
  final patch = JsonDiffPatcher.diff(sourceMap, targetMap);
  print('source: $sourceMap');
  print('target: $targetMap');
  print('patch: $patch');

  JsonDiffPatcher.applyPatch(sourceMap, patch);
  print('source=source+patch: $sourceMap');

  JsonDiffPatcher.applyPatch(sourceMap, patch.inverse());
  print('source=source-patch: $sourceMap');
}
